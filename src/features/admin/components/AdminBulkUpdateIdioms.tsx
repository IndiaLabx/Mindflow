...
import { useFetchIdiomsCountByFilter, usePerformIdiomBulkUpdate } from '../hooks/useAdminBulkUpdateIdioms';
...
const FIELDS = [
    ...
    { value: 'exam_year', label: 'Exam Year' },
...
export const AdminBulkUpdateIdioms: React.FC = () => {
    const { showToast } = useNotification();

    // Mutations
    const fetchCountMutation = useFetchIdiomsCountByFilter();
    const bulkUpdateMutation = usePerformIdiomBulkUpdate();
    ...
    const handleUpdate = async () => {
        ...
        const confirmMsg = `WARNING: You are about to update the '${targetField}' field to '${targetValue}' for ${affectedRows} Idiom entries.\n\nThis action cannot be undone. Type 'CONFIRM' to proceed.`;
        ...
        try {
            ...
            showToast({ title: "Update Success", message: `Successfully updated ${data.length} Idiom entries.`, variant: "success" });
            setTargetValue('');
            setAffectedRows(null);
        } catch (error: any) {
    ...
    return (
        ...
        >
            <div className="bg-amber-50/50 dark:bg-amber-900/10 rounded-2xl p-6 border border-amber-100 dark:border-amber-800/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select
                            ...
                            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all"
                        >
                            {FIELDS.map(f => (
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all placeholder:text-slate-400"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                        onClick={addFilter}
                        className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-orange-50 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400 text-sm font-medium hover:bg-orange-100 dark:hover:bg-orange-500/20 transition-colors"
                    >
                            ...
                            >
                                <div className="flex-1 w-full">
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none transition-all"
                                    >
                                        {FIELDS.map(f => (
                                ...
                                <div className="flex-1 w-full">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none transition-all placeholder:text-slate-400"
                                    />
                                </div>
                                <button
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        {affectedRows === null ? (
                            <div className="flex items-center gap-3 text-slate-300">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm opacity-80">Click preview to see how many Idioms will be affected.</p>
                                </div>
                            </div>
                        ) : (
                            <div className="flex items-center gap-3 text-amber-400">
                                ...
                                <div>
                                    <p className="font-bold text-lg">{affectedRows} Idioms Found</p>
                                    <p className="text-sm text-slate-300">Ready to update their <span className="font-mono text-amber-300">{targetField}</span> to <span className="font-mono text-amber-300">"{targetValue}"</span>.</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <button
                            ...
                            className="flex-1 md:flex-none px-6 py-3 rounded-xl bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-white font-bold shadow-lg shadow-amber-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {isUpdating ? (
