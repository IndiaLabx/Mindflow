...
import { useFetchOwsCountByFilter, usePerformOwsBulkUpdate } from '../hooks/useAdminBulkUpdateOWS';
...
export const AdminBulkUpdateOWS: React.FC = () => {
    const { showToast } = useNotification();

    // Mutations
    const fetchCountMutation = useFetchOwsCountByFilter();
    const bulkUpdateMutation = usePerformOwsBulkUpdate();
    ...
    const handleUpdate = async () => {
        ...
        const confirmMsg = `WARNING: You are about to update the '${targetField}' field to '${targetValue}' for ${affectedRows} OWS entries.\n\nThis action cannot be undone. Type 'CONFIRM' to proceed.`;
        ...
        try {
            ...
            showToast({ title: "Update Success", message: `Successfully updated ${data.length} OWS entries.`, variant: "success" });
            setTargetValue('');
            setAffectedRows(null);
        } catch (error: any) {
    ...
    return (
        ...
        >
            <div className="bg-indigo-50/50 dark:bg-indigo-900/10 rounded-2xl p-6 border border-indigo-100 dark:border-indigo-800/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select
                            ...
                            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all"
                        >
                            {FIELDS.map(f => (
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                        onClick={addFilter}
                        className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 text-sm font-medium hover:bg-purple-100 dark:hover:bg-purple-500/20 transition-colors"
                    >
                            ...
                            >
                                <div className="flex-1 w-full">
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition-all"
                                    >
                                        {FIELDS.map(f => (
                                ...
                                <div className="flex-1 w-full">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition-all placeholder:text-slate-400"
                                    />
                                </div>
                                <button
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        ...
                        {affectedRows === null ? (
                            <div className="flex items-center gap-3 text-slate-300">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm opacity-80">Click preview to see how many OWS will be affected.</p>
                                </div>
                            </div>
                        ) : (
                            <div className="flex items-center gap-3 text-indigo-400">
                                ...
                                <div>
                                    <p className="font-bold text-lg">{affectedRows} OWS Found</p>
                                    <p className="text-sm text-slate-300">Ready to update their <span className="font-mono text-indigo-300">{targetField}</span> to <span className="font-mono text-indigo-300">"{targetValue}"</span>.</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        ...
                        <button
                            ...
                            className="flex-1 md:flex-none px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white font-bold shadow-lg shadow-indigo-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {isUpdating ? (
