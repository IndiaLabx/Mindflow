...
import { useSynonymQuestionIndex, useSynonymFilterCounts, SynonymMetadata } from './hooks/useSynonymFilterCounts';
import { useSynonymProgress } from './hooks/useSynonymProgress';

interface SynonymsConfigProps {
    onStart: (data: SynonymWord[], filters?: InitialFilters, mode?: 'basic' | 'review') => void;
    onBack: () => void;
}
...
export const SynonymsConfig: React.FC<SynonymsConfigProps> = ({ onStart, onBack }) => {
    ...
    const [metadata, setMetadata] = useState<SynonymMetadata[]>([]);
    ...
    useEffect(() => {
        if (finalMatchingIds.length > 0 && finalMatchingIds.length <= 200) {
            ...
            const timer = setTimeout(async () => {
                try {
                    const data = await getFilteredSynonyms(filters, selectedAlphabets, sessionMode, finalMatchingIds);
    ...
    const handleSaveDeck = async () => {
        ...
        try {
            ...
            const data = await getFilteredSynonyms(filters, selectedAlphabets, sessionMode);
            if (data.length > 0) {
                 await deckService.createDeck('synonym', {
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            if (prefetchedDataRef.current?.idsHash === currentHash) {
                ...
                data = await getFilteredSynonyms(filters, selectedAlphabets, sessionMode, finalMatchingIds);
            }
            ...
            console.error("Error fetching full Synonyms data:", error);
            alert("Failed to load Synonyms data. Please try again.");
        } finally {
